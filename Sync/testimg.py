...
def get_output(imgs):
    ...
    for j,img in enumerate(imgs):
        ...
        if len(bboxes)>0:
          ...
          result.append({'filename':img,'bboxes':res_bbox})
    return result
#with open('./clue.json','w') as cf:
#    json.dump(get_output(glob(clue+'*.jpg')),cf)
#print('Writed clue bboxes info.')
with open('./plain.json','w') as pf:
    json.dump(get_output(glob(plain+'*.jpg')),pf)
print('Writed plain bboxes info.')
print("Done.")               
